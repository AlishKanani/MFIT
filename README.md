...
## Installation
The thermal RC model uses the Backward Euler Solver with SciPy's built-in sparse SuperLU solver. To install dependencies, run the following command:
```bash
...
- The thermal RC model uses a Backward Euler Solver with SciPy's sparse SuperLU, to solve the system of ODEs, which is faster than traditional explicit solvers.
...
python thermal_RC.py --material_prop_file <path to material property yaml file> \
                     ...
                     --xz_cuts <Y values for XZ cuts> \ # comma-separated (e.g., 1.5,3.0,5.5)
                     --yz_cuts <X values for YZ cuts> \ # comma-separated (e.g., 2.0,4.0)
                     --interactive_heatmaps {True,False} \ # generates interactive HTML for vertical cuts
                     --generate_3d_heatmap {True,False} # generates interactive 3D Plotly visualization

```
Note: For timing analysis, turn off the `generate_2d_heatmap` and `generate_DSS` flags.
